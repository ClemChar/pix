  ...
  describe('#getByCampaignIdAndCampaignParticipationId', () => {
    ...
    context('When there is an assessment for another campaign and another participation', () => {
      ...
      it('matches the given campaign and given participation', async () => {
        const campaignAssessmentParticipation = await campaignAssessmentParticipationResultRepository.getByCampaignIdAndCampaignParticipationId({ campaignId, campaignParticipationId });
      ...
      it('create CampaignAssessmentParticipation', async () => {
        const campaignAssessmentParticipation = await campaignAssessmentParticipationResultRepository.getByCampaignIdAndCampaignParticipationId({ campaignId, campaignParticipationId });

        expect(campaignAssessmentParticipation).to.be.instanceOf(CampaignAssessmentParticipationResult);
      });
    });
    ...
    context('When campaign participation is not shared', () => {
      ...
      it('does not fill results attributes', async () => {
        ...
        expect(campaignAssessmentParticipation.competenceResults).to.deep.equal([]);
        expect(campaignAssessmentParticipation.isShared).to.equal(false);
      });
    });
    ...
    context('When campaign participation is shared', () => {
      ...
      it('fills competenceResults', async () => {
        ...
        expect(campaignAssessmentParticipation.isShared).to.equal(true);
        expect(campaignAssessmentParticipation.competenceResults.length).to.equal(2);
        expect(campaignAssessmentParticipation.competenceResults).to.deep.equal(expectedResult);
      });
    });
  });
});
